 ...
 * @return string
 */
function theme_so7ob_get_main_scss_content($theme) {
    global $CFG;
    ...
    // Moove scss.
    $so7obvariables = file_get_contents($CFG->dirroot . '/theme/so7ob/scss/so7ob/_variables.scss');
    $so7ob = file_get_contents($CFG->dirroot . '/theme/so7ob/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/so7ob/scss/so7ob/_security.scss');

    $lastpreset = '';
    if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_so7ob', 'preset', 0, '/', $filename))) {
        $lastpreset = $presetfile->get_content();
    }

    // Combine them together.
    $allscss = $so7obvariables . "\n" . $scss . "\n" . $so7ob . "\n" . $lastpreset .    "\n" . $security;
 ...
 * @return string
 */
function theme_so7ob_get_extra_scss($theme) {
    $content = '';
 ...
 * @return string
 */
function theme_so7ob_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_so7ob_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/so7ob/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_so7ob_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    $theme = theme_config::load('so7ob');
    ...
    if ($filearea === 'hvp') {
        return theme_so7ob_serve_hvp_css($args[1], $theme);
    }
 ...
 */
function theme_so7ob_serve_hvp_css($filename, $theme) {
    global $CFG, $PAGE;
    ...
    $settings = new \theme_so7ob\util\settings();
    $content = $settings->hvpcss;

    $md5content = md5($content);
    $md5stored = get_config('theme_so7ob', 'hvpccssmd5');
